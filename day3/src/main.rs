...
const INPUT: &'static str = "...766.......821.547.....577......................................387.....................56..........446.793..........292..................
...
fn run(input: &str) {
    ...
    let mut sum = 0;
    let mut gear_adjacent = HashMap::new();
    for i in 0..lines.len() {
        let mut j = 0;
        while j < lines[0].len() {
            let ending = lines[i][j..]
                .iter()
                .enumerate()
                .find(|(_, c)| !c.is_digit(10))
                .map(|(i, _)| i)
                .unwrap_or(lines[i].len() - j);
            //dbg!(j, ending);
            let num_s = lines[i][j..j + ending]
                .into_iter()
                .try_fold(0, |a, b| Some(a * 10 + dbg!(b).to_digit(10)?));

            if let Some(num) = num_s && num != 0 {
                let mut any = false;
                let mut inserted_gear = false;
                for j in j..j + ending {
                    for (x, y) in adj {
                        let adj_x: Option<usize> = ((j as isize) + x).try_into().ok();
                        let adj_y: Option<usize> = ((i as isize) + y).try_into().ok();

                        match (adj_x, adj_y) {
                            (Some(adj_x), Some(adj_y))
                                if adj_x < lines[0].len() && adj_y < lines.len() =>
                            {
                                if !lines[adj_y][adj_x].is_digit(10) && lines[adj_y][adj_x] != '.' {
                                    any = true;
                                }
                                if lines[adj_y][adj_x] == '*' && !inserted_gear {
                                    gear_adjacent
                                        .entry((adj_y, adj_x))
