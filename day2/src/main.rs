...
const INPUT: &'static str = "Game 1: 4 blue, 7 red, 5 green; 3 blue, 4 red, 16 green; 3 red, 11 green
...
";
const TEST: &'static str = "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
fn main() {
    ...
    counts.insert("red", 12);
    counts.insert("green", 13);
    counts.insert("blue", 14);

    let mut sum = 0;
    let mut part2 = 0;
    for line in INPUT.lines() {
        let (_, id) = line.split_once(" ").unwrap();
        let (id, rem) = id.split_once(": ").unwrap();
        let id = id.parse::<u32>().unwrap();
        let mut drawn = HashMap::new();
        for subset in rem.split("; ") {
            for (count, cube) in subset.split(", ").map(|c| c.split_once(" ").unwrap()) {
                let count = count.parse::<u32>().unwrap();
                drawn.entry(cube).and_modify(|c| *c=std::cmp::max(*c, count)).or_insert(count);
            }
        }
        let mut possible = true;
        let mut part2_power = 1;
        for (cube, count) in drawn {
            println!("{cube} {count}");
            part2_power*=count;
            if counts[cube] < count {
                possible = false;
                //break;
            }
        }
        println!("part2_power {part2_power}");
        part2+=part2_power;
        if possible {
            sum+=id;
        }
